...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        acc, matrix = multimodal_classification(seed=seed, batch_size=16, epoch=20, dir_base=directory_base, n_classes=5)
        ...
        filepath = os.path.join(directory_base,
                                '/UserData/Zach_Analysis/result_logs/for_paper/paper_workspace/roberta_ai_vs_human_comparison_v44/confusion_matrix_seed' + str(
                                    seed) + '.xlsx')
        df.to_excel(filepath, index=False)
