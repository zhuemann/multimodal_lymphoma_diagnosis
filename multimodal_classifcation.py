...
def multimodal_classification(seed, batch_size=8, epoch=1, dir_base = "/home/zmh001/r-fcb-isilon/research/Bradshaw/",n_classes = 2, LR = 1e-6, beta1 = .9, beta2=.999):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=2700, eta_min=1e-6, last_epoch=-1,verbose=False) #5e-7
    best_acc = -1
    ...
    with torch.no_grad():
        ...
        #                            seed) + '.xlsx')
        filepath = os.path.join(dir_base, '/UserData/Zach_Analysis/result_logs/for_paper/paper_workspace/roberta_ai_vs_human_comparison_v52/predictions_seed' + str(
                                    seed) + '.xlsx')
